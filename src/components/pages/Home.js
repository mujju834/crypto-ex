...
const pair=[
    {name:"ETH",percent:3.19763724,price:2235.56},
    {name:"ETH",percent:3.19763724,price:2235.56},
    {name:"ETH",percent:3.19763724,price:2235.56},
    {name:"ETH",percent:3.19763724,price:2235.56},
    {name:"ETH",percent:3.19763724,price:2235.56},
...
function Home() {
  ...
  async function fetchEthereumPrice() {
    try {
      ...
      // Update the pair array with the real-time Ethereum price
      const updatedPair = Array(6).fill().map(() => ({
        name: "ETH",
        percent: 3.19763724, // Example percentage, update as necessary
        price: ethPrice, // Use the real-time Ethereum price
      }));

      setCoinData(updatedPair);
    } catch (error) {
